...
using ObjectStorageHelper_Universal.Model;

namespace ObjectStorageHelper_Universal.Services
{
    public class ObjectStorageHelper<T>
    {
        ...
        public void Delete(string fileName)
        {
            ...
            try
            {
                Monitor.Enter((object)(str = fileName), ref lockTaken);
                fileName = fileName + ".xml";
                try
                {
                    StorageFile result = this.GetFileIfExistsAsync(this.GetFolder(this.storageType), fileName).Result;
                    if (result == null)
                        return;
                    WindowsRuntimeSystemExtensions.AsTask(result.DeleteAsync((StorageDeleteOption)1)).Wait();
                }
                catch (Exception ex)
                {
                    var error = ex;
            ...
            try
            {
                ...
                try
                {
                    ...
                    IRandomAccessStream writeStream =
                        WindowsRuntimeSystemExtensions.AsTask<IRandomAccessStream>(
                        WindowsRuntimeSystemExtensions.AsTask<StorageFile>(
                        this.GetFolder(this.storageType).CreateFileAsync(
                        fileName, (CreationCollisionOption)1)).Result.OpenAsync((FileAccessMode)1)).Result;
                    try
                    {
                        using (Stream result = Task.Run<Stream>((Func<Stream>)(() => WindowsRuntimeStreamExtensions.AsStreamForWrite((IOutputStream)writeStream))).Result)
                            this.serializer.Serialize(result, (object)Obj);
                    }
                    finally
                    {
                        if (writeStream != null)
        ...
        public T Load(string fileName, T def)
        {
            bool lockTaken = false;
            string str = "";
            try
            {
                ...
                try
                {
                    IRandomAccessStream readStream = WindowsRuntimeSystemExtensions.AsTask<IRandomAccessStream>(
                        WindowsRuntimeSystemExtensions.AsTask<StorageFile>(
                        this.GetFolder(this.storageType).GetFileAsync(fileName)
                        ).Result.OpenAsync((FileAccessMode)0)).Result;
                    try
                    {
                        using (Stream result = Task.Run<Stream>(
                            (Func<Stream>)(() => WindowsRuntimeStreamExtensions.AsStreamForRead(
                                (IInputStream)readStream))).Result)

                            return (T)this.serializer.Deserialize(result);
                    }
                    finally
                    {
                        if (readStream != null)
